<?php
    ...
    $Usuario->getId();
    $same_pag="Contactos/lista_vn.php";
    ...
    $where= "";
    $limit=" LIMIT 250";
    $consulta_cl ="SELECT id_usuario, nombre, usuario from usuario WHERE IdPerfil= '19' and  estado ='Activo' order by Estado ;" ;
...
<head>
    ...
    <script src="resources/js/Contactos/contactos.js"></script>
    <style>
        *{
            padding: 0px;
            margin:0px;
...
<div class="modal fade" id="Modalcvn" tabindex="-1" aria-labelledby="Modalcvn" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <h5 class="modal-title" id="Modalcvn">Nuevo Contacto</h5>
            ...
            <div class="row">
                <div class="col-6 col-sm-4">
                    ...
                    <select class="form-control" placeholder="Seleccionar Asesor" name="asesores" id="asesores" onchange='asesores(this.value);'>
                        ...
                        <?php 
                            $consulta_cl ="SELECT id_usuario, CONCAT(nombre,' ', usuario) as usuarios from usuario WHERE IdPerfil= '19' and  estado ='Activo' order by nombre;" ;
                            $ccl= $catalogos->obtenerLista($consulta_cl);
                            while ($rowcl = mysqli_fetch_array($ccl)) {
    ...
    <table id="TContactos" class="table table-striped" style="width:100%">
                <thead>
                    <tr class='cabecera'>
                        ...
                        <th>Aseguradora</th>
                        <th>Hora</th>
                        <th><?php if($Asignado){?>Asignado<?php }?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                        $consulta ="SELECT
                                            c.*, u.nombre as asesor
                                        FROM
                                            contactos AS c
                           ...
                           <tr>
                                <td>
                                    ...
                                    <?php if ($Permisos->getModificacion()) {?>  
                                        <i class="bi bi-pencil-square" title="Editar" onclick="DatosEditar('<?php echo $rs['id_contactos'];?>')" style="font-size: 1rem; color: cornflowerblue; cursor:pointer;" data-bs-toggle="modal" data-bs-target="#Modalcvn" data-bs-whatever="@mdo"></i>
                                    <?php }?>
                                    <?php if ($Permisos->getBaja()) {?>
                                        <i class="bi bi-trash-fill" title="Eliminar" onclick="eliminar('<?php echo $rs['id_contactos'];?>')" style="font-size: 1rem; color: cornflowerblue; cursor:pointer;" ></i>
                                    <?php }?>
                                    <i class="bi bi-journal-album" title="Agendar" style="font-size: 1rem; color: cornflowerblue; cursor:pointer;" data-bs-toggle="modal" data-bs-target="#Modalagendar" data-bs-whatever="@mdo" onclick="SetIdcontacto('<?php echo $rs['id_contactos'];?>');"></i>
                                    <?php if ($rs['comentario_de_gerente']!="") {?>
                                        <i class="i bi-exclamation-triangle" title="Mensaje de gerente" onclick="myFunction(<?php echo $rs['id_contactos'];?>)" style="font-size: 1rem; color: red; cursor:pointer;" ></i>
                                    <?php }?>
                                    <div class="comentario_gerente" id="myDIV<?php echo $rs['id_contactos'];?>">
                                        <textarea readonly onclick="myFunctioncerrar(<?php echo $rs['id_contactos'];?>)"><?php echo $rs['comentario_de_gerente'];?></textarea>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <?php if ($Permisos->getConsulta()) {?>
                                        <select  name="Estatus" class="form-select emitir" onchange="CambiarEmitir('<?php echo $rs['id_contactos'];?>',this.value);">
                                        ...
                                        <textarea name='msjasesor' class='coment' id='msjasesor' ondblclick="GMA('<?php echo $rs['id_contactos'];?>',this.value);"><?php
                                            echo $rs['comentario'];
                                            ;?>
                                            </textarea>
                                ...
                                <?php if($comentarioG){?>
                                <td> 
                                    <textarea name='msgerencia' class='coment' id='msgerencia' ondblclick="GMG('<?php echo $rs['id_contactos'];?>',this.value);">
                                ...
                                <td><?php echo $rs['aseguradora']?></td>
                                <td><?php echo $rs['hora']?></td>
                                <td>
                                <?php if($Asignado){?>
                                    <select name="Usuario" class="form-select datos" aria-label="Default select example" id="Usuario" class="form-control" onchange="cambiarUser('<?php echo $rs['id_contactos'];?>',this.value)">
                                            <?php 
